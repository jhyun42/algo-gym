    ...
    for y in range(start_y, 4, -1):
        for x in range(4):
            ...
            for dir_idx in range(2):
                ...
                new_y = y + directions[dir_idx][0]
                new_x = x + directions[dir_idx][1]

                if new_y < 4 or new_x >= 4:
                    continue

                if BOARD[area_color][y][x] == BOARD[area_color][new_y][new_x]:
                    if dir_idx == 0:
                        block_type = 2
                    else:
                        block_type = 3
            ...
            if block_type == 1:
                move_point(y, x, area_color)

            elif block_type == 2:
                move_h_block(y, x, area_color)

            elif block_type == 3:
                move_w_block(y, x, area_color)
